...
import { searchItems, searchWorkflows, type ItemSearchFilters, type WorkflowSearchFilters } from "@/lib/api-search";
...
function coerceItemFilters(raw: unknown): Partial<ItemSearchFilters> {
  ...
  return {
    ...
    has_independent_replication: normalizeBoolean(value.has_independent_replication),
    has_mouse_in_vivo_validation: normalizeBoolean(value.has_mouse_in_vivo_validation),
...
export async function GET() {
  return NextResponse.json({
    ...
    llm_env: {
      api_key: "TOOL_DB_QUERY_LLM_API_KEY",
      base_url: "TOOL_DB_QUERY_LLM_BASE_URL",
      model: "TOOL_DB_QUERY_LLM_MODEL",
    },
  });
}
...
export async function POST(request: Request) {
  ...
  const promptInterpretation = prompt ? await interpretPromptQuery(prompt) : null;
  ...
  const itemResult = includeItems ? searchItems(itemFilters) : null;
  const workflowResult = includeWorkflows ? searchWorkflows(workflowFilters) : null;
