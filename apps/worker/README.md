...
The pipeline uses multiple layers of caching to avoid redundant work on re-runs:

| Layer                             | Where                                                                    | Key                                                                                                                           | Invalidated by                                                       |
| --------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **LLM response cache**            | `data/llm-cache/` (disk)                                                 | SHA-256 of full request body (model, messages, temperature)                                                                   | Prompt text change, evidence block change, model change              |
| **Extraction output dedup**       | Filesystem check                                                         | Job filename in `data/extractions/`                                                                                           | Deleting output file                                                 |
| **DOI dedup at harvest**          | DB lookup (`source_document` table)                                      | DOI or title                                                                                                                  | Never (additive only)                                                |
| **Packet ingest dedup**           | DB lookup (`extracted_packet.packet_fingerprint`)                        | SHA-256 of packet file content                                                                                                | File content changes (re-extraction)                                 |
| **First-pass load dedup**         | Same `extracted_packet.packet_fingerprint`                               | SHA-256 of packet file content                                                                                                | File content changes (re-extraction)                                 |
| **Materialization evidence hash** | `toolkit_item.materialization_evidence_hash` column                      | SHA-256 of claims, citations, validations, mechanisms, techniques, target_processes, item_type, and `MATERIALIZATION_VERSION` | New evidence ingested, schema change, `MATERIALIZATION_VERSION` bump |
| **LLM synthesis skip**            | DB check (`summary_derivation_model`, `item_explainer.derivation_model`) | Evidence hash unchanged + already has LLM-derived content                                                                     | New evidence ingested, `MATERIALIZATION_VERSION` bump                |
| **Replication summary memo**      | In-memory dict per `refresh()` call                                      | item_id                                                                                                                       | New `refresh()` call                                                 |

**When to force re-processing**:

- Bump `MATERIALIZATION_VERSION` in `pipeline_versions.py` to force all items through materialization and synthesis again.
- Delete `data/llm-cache/` to force all LLM calls to re-execute.
- Set `materialization_evidence_hash = NULL` on specific items to force their re-materialization.
- Small prompt tweaks that don't change evidence structure: usually OK to leave existing items as-is. The LLM cache key includes the prompt text, so only newly materialized items will get the updated prompt.
- Dramatic prompt or schema changes: bump `MATERIALIZATION_VERSION` so everything re-derives.

**Known caching gaps to watch for**:

- Comparisons are rebuilt whenever ANY item in the comparison set changed. If only 1 of 1,500 items changed, all comparisons rebuild. This is acceptable for now since comparisons are heuristic (no LLM cost), but a per-pair cache would help at 10K+ items.
- Gap-link LLM calls are cached at the request level, but the gap linker always re-fetches all gap/item profiles from DB. At scale, a staleness check before fetching would save DB I/O.
- Ingest and first-pass load now skip packets whose SHA-256 fingerprint is already in `extracted_packet`. This avoids replaying the full normalize/plan/execute flow for unchanged packets on re-runs. If a packet file is re-extracted (content changes), its new hash will not match and it will be re-ingested.
